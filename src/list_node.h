...
public:
    ...
    uint32_t* getPropertiesPtr();
    uint32_t getNeighborCount();
    uint32_t getNeighborMax();
    ptr getNextListNode();
    void setNextListNode(ptr next);
    bool containsNeighbor(uint32_t id);
    bool addNeighborWithProperty(uint32_t id, uint32_t property);
private:
    static const uint32_t neighborMax_ = 30; // number of neighbors per list node
